...
st.markdown(
    ...
    <div class="app-header">
        <div class="app-title">🏛️ BHU BHARATHI LAND FILES</div>
...
with st.sidebar:
    ...
    if st.button(
        ...
        key="sidebar_new_sheet"
    ):
        collect_data()
        new_sheet = next_sheet()
        st.session_state.database[new_sheet] = empty_file()
        save_database()
        clear_widgets(sheet)
        st.session_state.current_sheet = new_sheet
        st.session_state["show_saved_options"] = False
...
if navigation == "📊 Dashboard":
    ...
    if recent:
        rows = []
        for recent_sheet, recent_data in recent:
            rows.append({
                "Sheet": recent_sheet,
                "Document": recent_data.get("document_type", "SALE"),
                "Primary Name": recent_data.get("first_name", "") or "—",
                "Status": recent_data.get("booking_status", "STATUS PENDING"),
                "Saved": "Yes" if recent_data.get("saved", False) else "Draft"
            })
        ...
        st.info("No files have been saved yet.")
...
def person_form(
    ...
    if (
        ...
        name_column, dod_column = st.columns(2)
        ...
        with dod_column:
            ...
            try:
                dod_value = date.fromisoformat(
                    saved_dod
                )
            except (ValueError, TypeError):
                dod_value = date.today()

            selected_dod = st.date_input(
                "DOD",
                value=dod_value,
                min_value=date(1, 1, 1),
                max_value=date(9999, 12, 31),
                key=f"{sheet}_first_dod"
            )
